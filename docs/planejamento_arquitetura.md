...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 7h (UX do dashboard) construída e validada localmente.** Servidor deployado até **7e (migration `0007`)**; **7f + 7g + 7h pendentes de deploy**. Depois: **Fase 8 (n8n & Backups)** |
| Última atualização | 2026-07-21 |
| Bloqueios ativos | Nenhum |
| Próximo passo imediato | **1) Revisar/commitar a 7h e deployar 7f+7g+7h no servidor**: `git pull` → `cd infra` → `docker compose --env-file ../.env up -d --build backend frontend` → **`alembic upgrade head` (migration `0008`, obrigatória p/ a 7f)**; validar calendário→novo agendamento, consulta mensal, recorrência e cartões. **2) Rotacionar a credencial de teste antes versionada. 3) Depois, planejar a Fase 8 (§4.2).** |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-21):** Fases **0–7g ✅** e incremento **7h construído/validado localmente, ainda não commitado**. `main` remoto permanece no ponto documentado pelo commit `ed40be0`; **última migration = `0008`**.

- **Estado de deploy do servidor:** deployado e validado até a **7e (`alembic current` = `0007`)**. **7f (`65d8a7b`, migration `0008`), 7g (`5603392`) e a 7h local NÃO foram deployadas ainda** — ver "Próximo passo imediato".
...
- 2026-07-21 — [Fase 7h/Docs] 🟡 **UX do dashboard e saneamento de governança construídos e validados localmente.** Calendário→novo agendamento com data; Hoje antes do calendário; período mensal aplicado pela lupa; cadastros ativos depois das pendências; botão redundante removido. Credenciais/endereços/IDs removidos dos docs; rotação pendente no servidor; modelo de embedding encerrado; README e estados antigos reconciliados; verificação operacional de retenção OpenAI adicionada à Fase 9. `tsc` + build OK. **Aguarda revisão/commit/deploy junto de 7f+7g.**
