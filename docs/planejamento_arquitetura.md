...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 7 100% (7a–7g) ✅ construída/revisada/commitada.** Servidor deployado até **7e (migration `0007`)**; **7f + 7g pendentes de deploy**. Depois: **Fase 8 (n8n & Backups)** |
| Última atualização | 2026-07-21 |
| Bloqueios ativos | Nenhum |
| Próximo passo imediato | **1) Deployar 7f+7g no servidor** (o usuário faz): `git pull` → `cd infra` → `docker compose --env-file ../.env up -d --build backend frontend` → **`alembic upgrade head` (migration `0008`, obrigatória p/ a 7f)**; validar no browser (calendário, recorrência, cartões de paciente). **2) Depois, planejar a Fase 8 (§4.2)** — plano + perguntas de design (AskUserQuestion) ANTES de codar. |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-21):** Fases **0–7 ✅** — a Fase 7 (frontend) foi expandida em **7a–7g** e está feature-completa. `main` sincronizado, **push até `5603392`**. **Última migration = `0008`.** Nada não-commitado.

- **Estado de deploy do servidor:** deployado e validado até a **7e (`alembic current` = `0007`)**. **7f (`65d8a7b`, migration `0008`) e 7g (`5603392`, frontend) NÃO foram deployadas ainda** — ver "Próximo passo imediato".
...
**Próximo passo imediato (1): DEPLOY da 7f+7g** — o **usuário** roda no servidor (uma linha por vez, de dentro do projeto): `git pull` · `cd infra` · `docker compose --env-file ../.env up -d --build backend frontend` · `docker compose --env-file ../.env exec backend alembic upgrade head` (**→ `0008`, obrigatória**). Validar no browser via túnel (`8090`): calendário (dias coloridos, "Ir para hoje"), recorrência (criar série + desfazer), cartões de paciente (observações inline), fonte/espaçamento.
...
**Contexto de ambiente/dados (servidor):** SPA em `http://127.0.0.1:8090` (localhost do servidor; acesso via **túnel SSH/Tailscale** rodado **na máquina do usuário, não no servidor**: `ssh -L 8090:127.0.0.1:8090 hades@100.121.137.44` — IP Tailscale do servidor; nome curto `hadesserver` NÃO resolve fora dele; erro `Address already in use` = túnel rodado dentro do próprio servidor). **`.env` do servidor tem `COOKIE_SECURE=false`, `FRONTEND_HOST_PORT=8090`, `OPENAI_API_KEY` ATIVA.** Backend interno `8000` (host `8010`). **Todos os módulos preenchidos** (+ `dashboard`; anonimizacao, evolucoes, llm, rag-embutido em evolucoes). Dados de teste (descartáveis): psicóloga `teste@clinica.local` / `SenhaForte123`; paciente **`b0707184-d983-4301-b4cc-dac552494284`** ("Crianca RAG", **TCLE ATIVO**, já com evoluções+embeddings → bom p/ testar RAG); paciente `316b1a57-…` (TCLE **revogado** → bom p/ testar o bloqueio §2.2). **⚠️ p/ testar a criação de evolução (7e+):** o paciente precisa de um agendamento **`realizado`** (marcar na agenda) — senão o editor não lista atendimento p/ vincular. **Setup de teste local (WSL):** venv py3.12 em scratchpad com `pip install -e '.[dev]'` p/ `pytest tests/unit`; FastAPI 0.139 usa inclusão **lazy** de routers (`app.routes` não expande — checar rotas via `app.openapi()['paths']`). Detalhes na seção **Operação & Deploy**.
