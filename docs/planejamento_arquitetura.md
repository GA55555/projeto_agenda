...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 7d (fechamento da 7) 🟡 construída + revisada local — aguarda commit/deploy.** Depois: **Fase 8 (n8n & Backups)** |
| Última atualização | 2026-07-20 |
| Bloqueios ativos | Nenhum |
| Próximo passo imediato | **Commit + deploy da 7d** (ações agenda, /dashboard/resumo, perfil): `up -d --build backend frontend` + **`alembic upgrade head` (migration `0006`, obrigatória)**. Validar no browser: ações na agenda (409 em transição inválida), dashboard 3 blocos, /perfil (trocar e-mail exige senha; evento na auditoria). Depois planejar a **Fase 8 (§4.2)** — plano + perguntas de design ANTES de codar. |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-20):** Fases **0, 1, 2, 3, 3.5, 4, 5, 6, 7 ✅** concluídas e validadas no servidor — **incluindo a sub-fase 7c (melhorias de frontend)**. O produto está **funcional ponta a ponta no browser**: login (cookie httpOnly) → dashboard → agenda → ficha → **gerar evolução com IA** (túnel opaco §2.3 + RAG §3.2) → aprovar/gravar; + cadastros (agendar, criar paciente via wizard, responsáveis). Branch `main` sincronizado (push até **`cbaebdf`**). Última migration = **`0005`**. Sem trabalho pendente/não-commitado.

**Próxima ação:** planejar a **Fase 8 (Automação n8n & Backups, §4.2)** — plano + perguntas de design ANTES de codar. Característica diferente das anteriores: boa parte vive **dentro do n8n** (já rodando no servidor), não no código Python. Escopo: webhook FastAPI→n8n **autenticado** disparado **após a aprovação/assinatura** de uma evolução; OAuth2 do Google **100% no n8n** (Drive+Docs API; app/BD nunca tocam senhas Google); JSON limpo (não-anonimizado) → PDF/Sheets no diretório encriptado da psicóloga; rotina diária `pg_dump` + arquivamento de **WAL** no **HDD 500 GB**; teste de restauração. *Decisão em aberto relacionada: assinatura eletrônica FORMAL da evolução (hoje "aprovar e gravar" = aprovação auditável via `autor_usuario_id`+timestamp) — foi adiada p/ §8/§9; reavaliar aqui.*
...
**Contexto de ambiente/dados (servidor):** SPA em `http://127.0.0.1:8090` (localhost do servidor; acesso via **túnel SSH/Tailscale**: `ssh -L 8090:127.0.0.1:8090 hades@100.121.137.44` — o IP Tailscale do servidor; nome curto `hadesserver` NÃO resolve fora dele). **`.env` do servidor tem `COOKIE_SECURE=false`, `FRONTEND_HOST_PORT=8090`, `OPENAI_API_KEY` ATIVA.** Backend interno `8000` (host `8010`). **Todos os módulos preenchidos** (anonimizacao, evolucoes, llm, rag-embutido em evolucoes). Dados de teste (descartáveis): psicóloga `teste@clinica.local` / `SenhaForte123`; paciente **`b0707184-d983-4301-b4cc-dac552494284`** ("Crianca RAG", **TCLE ATIVO**, já com evoluções+embeddings → bom p/ testar RAG); paciente `316b1a57-…` (TCLE **revogado** → bom p/ testar o bloqueio §2.2). Detalhes na seção **Operação & Deploy**.
...
- **7c.2 — Cadastros:** criar **agendamento** (form + 409 de sobreposição); **Responsáveis** (lista + criar/editar + detalhe com contato: nome, cpf, nascimento, telefone, e-mail, endereço); criar **paciente** (wizard responsável→dados→TCLE). Endpoints já existem (`POST /agendamentos`, `/responsaveis`, `/pacientes`). → 🟡 **construída + revisada + validada local (2026-07-20)**; `tsc`+`build` OK, backend 61 unit tests. Code-review (4 achados aplicados): **CPF duplicado agora → 409** (antes 500; `responsaveis.service` captura `IntegrityError` sqlstate 23505 → `CpfDuplicado` → 409); wizard não recria responsável no retry; guard de datas no agendamento; tipo `ResponsavelUpdate` restrito. Teste de integração do 409 adicionado. Aguarda deploy.
- **7c.3 — Dashboard:** visão geral — hoje na agenda, contadores, acesso rápido. → 🟡 **construída + revisada + validada local (2026-07-20)**; `tsc`+`build` OK. Compõe 3 GETs (agenda-hoje, pacientes, responsáveis) **sem N+1**; stat tiles + agenda de hoje + ações rápidas; **Dashboard vira a landing** (`/`). Code-review (1 aplicado): `Promise.allSettled` p/ a landing degradar graciosamente. **Pendências (sem TCLE / embeddings pendentes) NÃO entraram** — exigiriam N+1; ficam p/ um futuro `GET /dashboard/resumo` (agregado no backend). Aguarda deploy.
...
**Escopo (6 blocos):** (0) bugfix 500 no PATCH /auth/me *(diagnóstico: provável migration 0006 não aplicada — aguarda saída do servidor)*; (1) dados & validações; (2) agenda UX; (3) arquivar/apagar paciente; (4) dashboard histórico; (5) responsividade.
...
**Code-review alto esforço (8 finders + verificação) → 10 achados + itens cortados APLICADOS:** (1+2) **evolução só vincula atendimento `realizado`** (impede data futura/no-show E torna impossível apagar agendamento com prontuário — conjuntos disjuntos; editor filtra realizados; `apagar()` ainda traduz FK-violation→409 por defesa); (3) **wizard passou a coletar nascimento do responsável** com trava 18+ (antes contornava a regra); (4) **duração customizável** no agendamento (grade travava ≤60min → regressão); (5) **coerência tile×lista do dashboard**: `resumo` expõe `dia_inicio`/`dia_fim` (fuso da clínica) e a agenda do dia usa essa janela (browser não recalcula o dia); (6) editor **zera seleção/rascunho ao trocar de paciente** (`useEffect [id]`); (7) `dataMaximaMaioridade` alinhada ao backend no 29/02; (8) **pacientes buscado 1×** (dep `[]`) no dashboard e no agendamento (era refetch por dia/mês) + 3 contagens sobre `pacientes` consolidadas em 1 query (`COUNT FILTER`); (9) índice `(tenant_id, criado_em)` em `evolucoes` (migration 0007); (10) helpers extraídos p/ `utils/format.ts` (`hojeISO`/`mesAtualISO`/`fmtHora`/`toISODate`) e `utils/status.ts` (`ROTULO_STATUS`), predicado de sobreposição único. **Cortados aplicados:** `_to_out` kwarg obrigatório; `conflito` sem `useMemo`; wrapper de ação único na Ficha; docstring "nunca DELETE" corrigida. **99 unit tests**, `tsc`+build OK, SQL 0007 (2 índices + FK + grants) renderizado. → 🟡 **construída + revisada + validada local. Aguarda commit/deploy (`alembic upgrade head` → 0007).**

**Code-review alto esforço (8 finders + verificação) → 10 achados APLICADOS:** (#1) **migration `0006`**: `GRANT UPDATE (nome,email,senha_hash,atualizado_em) ON usuarios TO agenda_app` — sem ela o perfil daria 500 (0002 só dava SELECT; menor privilégio §2.1.1); (#2) limite de senha em **72 bytes** UTF-8 (bcrypt ≥5 levanta ValueError, não trunca) — validador no schema + aviso na SPA; (#3) **e-mail normalizado minúsculo** no `PerfilUpdate` + login case-insensitive (`func.lower`) — evita lockout e case-variant burlar o UNIQUE; (#4) **máquina de estados no backend** (`validar_transicao_status`): `cancelado` é terminal, PATCH não entra/sai de cancelado (só rota `/cancelar`, e só de `agendado` — re-cancelar não sobrescreve mais o motivo) → 409; (#5) tile "Atendimentos hoje" deriva da MESMA busca da lista (coerência tile×lista; resumo é fallback); (#6/#7) ações da agenda **sem refetch**: aplicam o retorno do PATCH via overrides locais (mata race de duplo-clique e o apagão da tabela em falha de reload); (#8) `refresh()` fora do try do save (falha do GET não vira "falha ao salvar"); (#9) **trocar e-mail exige senha atual** (re-auth) + **evento `perfil_email_alterado` na auditoria** append-only §2.2 (PATCH /auth/me sob `get_tenant_session`); (#10) predicado "TCLE vigente" extraído p/ `clausula_consentimento_ativo` (fonte única em `consentimentos.service`, usada pelo gate E pelo dashboard). **88 unit tests** (+15), `tsc`+build OK, SQL da 0006 renderizado offline. → 🟡 **construída + revisada + validada local. Aguarda commit/deploy.**
...
- 2026-07-20 — [Fase 7c] ✅ **Sub-fase Melhorias de Frontend CONCLUÍDA e validada no servidor.** Login→dashboard→cadastros percorridos no browser. **Hotfix pós-deploy:** `/auth/me` deu **500** (`PerfilOut.email` era `EmailStr`; `email-validator` rejeita o TLD reservado `.local` do usuário de teste `teste@clinica.local` na SERIALIZAÇÃO da resposta) → corrigido: response models (`Out`) usam `str`, `EmailStr` só na entrada (`Create`/`Update`). Aplicado a `PerfilOut`/`UsuarioOut`/`ResponsavelOut`. Commit `cbaebdf`. Guarda: `test_auth_api` já usa e-mails `.local` (só não roda no deploy). Detalhe da construção 7c.1/7c.2/7c.3 abaixo. ↓
