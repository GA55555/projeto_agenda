...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 8b (webhook autenticado).** Contrato HTTP aprovado, mas receptor n8n mantido inativo por execuções que não finalizam e persistem payload. |
| Última atualização | 2026-07-29 |
| Bloqueios ativos | **n8n 2.30.5 não finaliza as execuções do webhook**, inclusive com correção upstream de timestamp e task runner externo; elas permanecem `running` e contrariam a proibição de persistir payload clínico. Não realizar teste real nem manter o workflow ativo. |
| Próximo passo imediato | Diagnosticar a finalização das execuções no n8n com payload exclusivamente sintético. Só reativar depois de provar `0` execuções e `0` payloads persistidos após os cinco casos do contrato. |
