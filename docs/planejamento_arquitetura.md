...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 8b (automação/exportação).** Webhook e PDF padronizado aprovados; falta OAuth2, pasta privada e upload idempotente no Google Drive. |
| Última atualização | 2026-08-01 |
| Bloqueios ativos | Passo 4 depende da psicóloga criar a pasta privada no Drive e iniciar a credencial Google Drive OAuth2 no n8n. A confirmação formal de ZDR permanece tarefa da Fase 9. |
| Próximo passo imediato | Criar a pasta privada no Drive; no n8n abrir **Credentials → Create Credential → Google Drive OAuth2 API** e copiar somente o **OAuth Redirect URL** (não é segredo). |
