...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 8b (webhook autenticado).** Contrato e retenção aprovados em n8n 2.33.0 com runner externo; receptor mantido inativo até a rotação dos segredos expostos no diagnóstico. |
| Última atualização | 2026-07-31 |
| Bloqueios ativos | **Rotação operacional de segredos.** O diagnóstico exibiu valores do contêiner; rotacionar segredo HMAC, token do runner e credenciais afetadas antes de qualquer evento real. |
| Próximo passo imediato | Rotacionar os segredos com backup prévio, retirar logging `debug`, repetir os cinco casos sintéticos e somente então publicar o workflow para o primeiro teste real controlado. |
