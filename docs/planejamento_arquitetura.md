...
| Bloqueios ativos | Segredos n8n/HMAC rotacionados, mas a última matriz HTTP precede a rotação de 2026-08-05 e deve ser repetida de forma sintética antes de futura publicação; o receptor permanece despublicado. A LAN física está contida de forma persistente em IPv4 e IPv6 nas seis portas administrativas; o reteste externo preservou o acesso pela Tailscale. Os bindings continuam amplos e Homarr permanece legado com Docker socket gravável; seu backup/rollback está comprovado, mas a stable `v1.73.0` candidata foi bloqueada antes da execução por `2` achados críticos e `19` altos corrigíveis. Os listeners externos `139/445` foram atribuídos ao Samba do host e `3000` ao processo PM2 `mochila` do projeto Ascensão; o teste LAN confirmou bloqueio, mas qualquer restrição adicional depende dos responsáveis. ZDR OpenAI não está provisionado, porém o operador aceitou temporariamente esse risco; isso não substitui os demais gates de go-live. Scans de imagens/segredos foram concluídos: frontend corrigido e implantado está limpo, mas imagens upstream de backend/n8n/runner/PostgreSQL/pgvector mantêm CVEs que exigem correção do fornecedor ou exceção formal. O frontend ainda tem exceção temporária para advisory RSC do React Router, caminho não usado pela SPA. |
| Próximo passo imediato | **Go-live continua bloqueado pelos gates restantes:** registrar backup/verificação manuais no `journald` local, sem timer e sem alerta externo; aguardar nova release corrigida do Homarr sem executar `v1.73.0`; aceitar/revisar exceções de CVE e repetir matriz sintética pós-rotação antes de qualquer publicação. O restore em host/VM separado foi recusado pelo operador; manter a limitação documentada. |
...
O bloqueio por `2` críticas e `19` altas corrigíveis permanece vigente.
**Decisão do operador:** aguardar nova release corrigida; manter o legado preservado e
não executar a Fase B até nova rechecagem oficial e scan completo.
...
- [~] Registro de exceções temporárias em
  ...
  pendente.
- [~] ZDR OpenAI não está provisionado; risco aceito temporariamente pelo operador em
  2026-08-06 e registrado como `EXC-03` em [`excecoes_risco_2026-08-05.md`](./excecoes_risco_2026-08-05.md). Os controles de Sharing e `API call logging` estão `Disabled`; `store=false` continua aplicado onde cabível. A aceitação não comprova ZDR ativo nem libera dados reais sem os demais gates.
- [~] Observabilidade mínima: `infra/observabilidade/verificar_runtime.sh` monitora
  ...
  resultado registrado no `journald` local com a tag `agenda-observabilidade`.
...
- 2026-08-06 — [Fase 9/Homarr] 🔴 **Rechecagem sem mudança.** A release oficial latest continua `v1.73.0`; não houve nova imagem, candidato ou alteração operacional. O scan anterior (`2` críticas e `19` altas corrigíveis) continua bloqueando a Fase B.
