...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 8b (webhook autenticado).** Emissor commitado; tabela antirreplay aplicada e receptor n8n criado/importado inativo. |
...
| Próximo passo imediato | Configurar `AGENDA_WEBHOOK_SECRET` e `NODE_FUNCTION_ALLOW_BUILTIN=crypto` no n8n, criar/selecionar credencial PostgreSQL mínima no nó antirreplay e executar os cinco casos sintéticos antes de ativar. |
