...
| Bloqueios ativos | **Ativação do webhook/workflow bloqueada** até o conjunto n8n do snapshot `aa64be52` ser restaurado e validado isoladamente. Inventário e desenho permanecem liberados. |
| Próximo passo imediato | Restaurar isoladamente o PostgreSQL e o volume persistente do n8n a partir do snapshot `aa64be52`, usando rede/volumes exclusivos; validar chave, credenciais e workflows sem dispará-los. |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-28):** o n8n foi inventariado e incluído no recovery coordenado. O snapshot `aa64be52` validou dump/globals dos dois PostgreSQL, documentos, volume persistente e chave/configuração do n8n, fonte e checksums; todos os serviços voltaram saudáveis e o staging foi desmontado. A assinatura explícita e imutável está implantada na migration `0011`. **Retomar por:** restaurar o conjunto n8n isoladamente, sem rede externa e sem ativar workflows; segunda cópia offline/off-site, retenção, RTO e restore em host separado permanecem pendentes de hardening.
...
- [~] Incluir banco, volume persistente e cópia root-only da chave no backup coordenado. *(Snapshot `aa64be52` criado e validado em 2026-07-28; restore isolado pendente.)*
...
- 2026-07-28 — [Fase 8b] 🟡 **n8n incluído e provado no backup coordenado.** n8n 2.30.5 em localhost, PostgreSQL 16 dedicado, volumes separados e chave explícita confirmados sem expor valores. Cópia root-only corresponde à chave ativa. O snapshot `aa64be52` contém e validou dump/globals, volume, configuração efetiva, chave, imagens/IDs e checksums; Agenda e n8n voltaram saudáveis, staging desmontado. Commit `42c374c` local, sem push. Restore isolado do n8n ainda bloqueia o webhook.
