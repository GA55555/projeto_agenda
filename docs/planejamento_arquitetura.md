...
| Próximo passo imediato | Implantar a correção do wrapper, repetir o backup integrado, tratar o staging incompleto preservado e salvar o bundle das imagens aprovadas. Depois executar restore isolado e medir o RTO. |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-25):** a **Fase 7 foi concluída**. Na **Fase 8a**, o HDD compartilhado `/mnt/dados` permaneceu intacto; staging gocryptfs e repositório Restic local foram configurados sem senhas persistidas. O primeiro conjunto coordenado incluiu banco, documentos, código clonável, metadados e configurações; hashes, estruturas e `restic check` passaram. O wrapper foi implantado e configurado, mas a primeira prova integrada travou no `docker compose exec` antes de iniciar `pg_dump`; a proteção reiniciou os serviços, preservou o conjunto incompleto e desmontou o staging. A correção troca essa etapa por `docker exec` direto, aplica timeout com encerramento forçado e informa progresso. Restore isolado e segunda cópia offline/off-site continuam pendentes.

- **Estado de deploy do servidor:** código **`869be25`**, migration **`0010 (head)`**, PostgreSQL/backend/frontend saudáveis; healthcheck IPv4 aplicado e wrapper manual configurado.
...
**Próximo passo imediato: FASE 8a — RECOVERY.** Implantar a correção que usa `docker exec` direto, repetir a prova integrada do wrapper e remover somente o staging incompleto após inspeção. Salvar o bundle das imagens implantadas. Em seguida restaurar o conjunto em ambiente isolado, conferir RLS/hashes/coerência banco↔documentos e medir o RTO. A rotação da credencial anteriormente versionada continua obrigatória.
