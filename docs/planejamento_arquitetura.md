...
| Campo | Valor |
| --- | --- |
| Fase corrente | **Fase 7i (busca, listas compactas e arquivamento) construída e validada localmente; ainda sem commit/push/deploy.** Servidor deployado até **7e (migration `0007`)**; **7f–7i pendentes de deploy**. 7j (controle de sessões) e 7k (documentos) já estão delimitadas, mas não iniciadas. |
| Última atualização | 2026-07-22 |
| Bloqueios ativos | Nenhum |
| Próximo passo imediato | **Revisar a entrega local da 7i** (busca/ordenação, aba de arquivados, cartões compactos, bloqueio de arquivamento e migration `0009`). Após aprovação: code-review, commit/push quando solicitado e deploy conjunto 7f–7i (`alembic upgrade head` → `0009`). Só então iniciar a **7j**, mantendo uma subfase por vez. A rotação da credencial antes versionada continua obrigatória antes do próximo uso do servidor. |

> Atualize esta tabela ao fim de cada sessão de trabalho.

### 🔖 Ponto de Retomada (ler primeiro na próxima sessão)

**Onde paramos (2026-07-22):** a **7i foi construída e validada localmente**, ainda sem commit/push/deploy. `main`, `origin/main` e `HEAD` permanecem sincronizados em **`36e538f`** antes destas mudanças; a árvore de trabalho contém a entrega da 7i e a nova **migration `0009`**.
...
**Próximo passo imediato (1): REVISAR E FECHAR A 7i** — aplicar os achados do code-review, repetir a validação local e apresentar o diff. Após aprovação do usuário: commit/push quando solicitado; antes do próximo uso do servidor, rotacionar a credencial de teste anteriormente versionada.

**Próximo passo imediato (2): DEPLOY CONJUNTO 7f–7i** — no servidor, depois do push: `git pull` · `cd infra` · `docker compose --env-file ../.env up -d --build backend frontend` · `docker compose --env-file ../.env exec backend alembic upgrade head` (**→ `0009`, obrigatória**). Validar calendário/recorrência (7f), cartões (7g), dashboard (7h) e busca/arquivamento/reagendamento (7i). Depois iniciar **7j** e, concluída ela, **7k**. A Fase 8 permanece posterior a essas subfases aprovadas.
