"""Testes da extracao de token (cookie httpOnly x header) — Fase 7.

Sem BD: exercita `_extrair_token`/`get_current_user` com um Request falso e um
JWT real (o segredo de teste vem do conftest).
...
def test_get_current_user_valida_jwt_do_cookie():
